...
namespace PersonControll.Controllers
{
    ...
    {
        public FileResult GetReport(string surname, string firstname, string lastname, string firstBirthday, string lastBirthday)
        {
            ...
            List<Person> citizens = connect.GetDataTable(sql.GetAllColumn(surname, firstname, lastname, first, last));

            Stream stream = new MemoryStream();
            WebReport webReport = new WebReport();
            webReport.Report.Load(this.Server.MapPath("~/FastRep/CitizenFastReport.frx"));
